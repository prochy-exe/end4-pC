...
NestableObject {
    ...
    function save(index = -1) {
        if (index >= 0) {
            root.saveMonitor(index)
            return
        }
    ...
    function saveMonitor(index) {
        ...
        const fields = root._fieldsToWrite(m, changedKeys)
        if (Object.keys(fields.setPairs).length === 0 && fields.resetKeys.length === 0) return

        let args = ["python3", root.configuratorScriptPath, "--file", root.monitorsLuaPath, "--output", m.name]
        for (const key in fields.setPairs) args.push("--set", key, String(fields.setPairs[key]))
        for (const key of fields.resetKeys) args.push("--reset", key)
